...
- Marks local HEAD with a dedicated themeable yellow ring-and-center-dot graph node and matching inline symbol without a literal `HEAD` label, uses a subdued neutral tag color, groups every co-located branch's HEAD, local, and remote indicators without adding labels beyond the current branch and compact orientation anchors, and shows the short commit hash when detached
