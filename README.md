...
- Shows a compact graph, non-wrapping subject, date, and compact ref indicators with complete names in tooltips
- Marks the checked-out position with an icon-and-text `HEAD` badge; local refs are filled, remote refs are outlined, and tags use a distinct shape
