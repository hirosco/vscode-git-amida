...
- Marks local HEAD with a ring-and-center-dot graph node, labels it with the checked-out branch or short commit hash when detached, labels each locally recorded remote default branch by its target name without a redundant `HEAD` marker, retains `main` or `master` as fallback orientation anchors, and keeps other refs compact through fill and shape
