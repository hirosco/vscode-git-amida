...
- Opens single-commit, Range, and Selection comparisons in one reusable native preview diff on single-click or keyboard navigation; double-click, Enter, and **Open Changes** pin text, supported-image, and native binary-fallback comparisons, while replaced previews and closed pinned tabs release their virtual resources
...
- Passes other binary files to the editor's native binary fallback without decoding them as text, while keeping submodules and text blobs beyond the current VS Code/Cursor `diffEditor.maxFileSize` setting visible and labeled
