...
- Generate the realistic AI fan-out fixture with `node scripts/create-ai-parallel-repository.mjs /absolute/path/to/git-amida-ai-parallel-demo`, then validate it with `node scripts/validate-ai-parallel-repository.mjs /absolute/path/to/git-amida-ai-parallel-demo`. Its eight linked worktrees live under the sibling `<target>-agents` directory, and both generated locations must be removed explicitly before regeneration.
- Generate dense graph fixtures separately with `node scripts/create-graph-stress-repository.mjs /absolute/path/to/git-amida-graph-stress-demo`, then validate them with `node scripts/validate-graph-stress-repository.mjs /absolute/path/to/git-amida-graph-stress-demo`. Keep pathological branch density out of the normal screenshot repository.
