...
- Mark local HEAD with a ring-and-center-dot graph node and matching inline symbol in the yellow-family GitAmida-specific `gitAmida.headRefColor`, independent of graph-lane color. Use the subdued neutral `gitAmida.tagRefColor` for tags while retaining VS Code's SCM Graph colors for local and remote refs. Do not render a literal local `HEAD` label; use the checked-out branch name, or the short commit hash when detached.
