...
- Mark local HEAD with a ring-and-center-dot graph node in its graph-lane color so branch flow remains continuous. Use the same shape for the inline symbol, colored with the yellow-family GitAmida-specific `gitAmida.headRefColor`. Use the subdued neutral `gitAmida.tagRefColor` for tags while retaining VS Code's SCM Graph colors for local and remote refs. Do not render a literal local `HEAD` label; use the checked-out branch name, or the short commit hash when detached.
...
- Include unresolved index paths in the working-tree count even when their saved filesystem content matches HEAD. Whenever conflicts exist, supplement the row with their count and group the inspection list as Merge Changes and Changes. Label merge, rebase, cherry-pick, and revert operations when their reliable Git markers exist; keep stash application and other unclassified conflicts generic rather than guessing their origin.
- Open content conflicts with both index sides as their actual working-tree files so VS Code or Cursor owns conflict markers, Merge Editor, and host-specific actions. Keep modify/delete conflicts selectable but do not open a preview; on an explicit open attempt, explain that Source Control must resolve the conflict and offer to open it.
...
- Expose the external Git mergetool only when it is configured and a standard actual-file content-conflict editor with both index sides is active. Treat it as a secondary alternative to entering the built-in Merge Editor, and hide it after the Merge Editor opens. Never expose it from Changed-files context menus, modify/delete conflicts, or the Command Palette. Revalidate the live conflict and unsaved editor state, confirm that the tool may modify and stage the file or create a backup, then refresh after the configured tool exits.
...
- Regenerate the separate synthetic demo repository with `node scripts/create-demo-repository.mjs /absolute/path/to/vscode-git-amida-demo` for comprehensive manual validation and publication screenshots. The generator refuses to overwrite its main repository, component source, branch-backed review worktree, or detached agent worktree; remove those exact disposable targets explicitly before regenerating.
- Build GitAmida, then run `node scripts/validate-demo-repository.mjs /absolute/path/to/vscode-git-amida-demo` to verify the generated history through the production Git adapter before manual editor testing.
- Generate the realistic AI fan-out fixture with `node scripts/create-ai-parallel-repository.mjs /absolute/path/to/vscode-git-amida-ai-parallel-demo`, then validate it with `node scripts/validate-ai-parallel-repository.mjs /absolute/path/to/vscode-git-amida-ai-parallel-demo`. Its eight linked worktrees live under the sibling `<target>-agents` directory, and both generated locations must be removed explicitly before regeneration.
- Generate dense graph fixtures separately with `node scripts/create-graph-stress-repository.mjs /absolute/path/to/vscode-git-amida-graph-stress-demo`, then validate them with `node scripts/validate-graph-stress-repository.mjs /absolute/path/to/vscode-git-amida-graph-stress-demo`. Keep pathological branch density out of the normal screenshot repository.
- Generate unresolved merge and rebase fixtures with `node scripts/create-conflict-demo-repository.mjs /absolute/path/to/vscode-git-amida-conflict-demo`, then validate them with `node scripts/validate-conflict-demo-repository.mjs /absolute/path/to/vscode-git-amida-conflict-demo`. The main target remains paused at merge conflicts and the sibling `<target>-rebase` worktree remains paused at a rebase conflict; remove both exact disposable paths before regeneration.
- Generate unresolved cherry-pick, revert, and stash-apply fixtures with `node scripts/create-operation-conflict-demo-repositories.mjs /absolute/path/to/vscode-git-amida-conflict-demo`, then validate them with `node scripts/validate-operation-conflict-demo-repositories.mjs /absolute/path/to/vscode-git-amida-conflict-demo`. The generator creates the three standalone sibling repositories `<target>-cherry-pick`, `<target>-revert`, and `<target>-stash`; remove those exact disposable paths explicitly before regeneration.
