...
Color file paths with the same VS Code Git decoration token as their displayed status. Keep the Status column limited to complete Git status wording. Supported images and other binary content use their bundled icons without redundant visible labels; their accessible descriptions still identify the content kind. Show compact neutral outline tags beside the path for oversized text, submodules, and canonical Git LFS pointers because those facts are independent from Git status and may coexist.

Changed-file context actions may open the exact current `file.path` in the working tree or copy its file name or repository-relative path. Revalidate the row against current Extension Host state before every action. Opening rejects missing paths, non-files, symbolic links, and paths that resolve outside the repository, then offers File History instead of guessing `oldPath`, searching for a rename, or opening another same-named file.
...
Each File History tab shows the available revisions that changed the file, follows renames where Git can determine them, and retains the path identity needed to compare historical blobs. The path and status use the same Git decoration color as Changed files, while commit subjects and dates stay neutral so color continues to describe the file transition rather than the whole commit. Show the same compact neutral outline tags for oversized text, submodules, and canonical Git LFS pointers beside each revision path. Derive those tags from the revision's actual before and after object IDs and modes, using batched metadata and small-pointer reads when the tab loads rather than attributes or one Git process per row. A resizable right column shows the selected revision's commit metadata using the same fields as Repository History; it does not duplicate Changed files because the left side already represents one file's revisions. Opening from a historical Changed-files row selects and reveals that row's revision initially without truncating later file history. Reopening the same file focuses its existing tab and moves to the requested revision.
...
Content conflicts with both index sides open the actual working-tree file rather than a GitAmida virtual diff. This delegates conflict markers, the optional Merge Editor, and host-specific actions such as Cursor's **Resolve in Chat** to the active editor. A modify/delete conflict remains selectable without opening an editor on preview; an explicit Enter, double-click, or **Open Changes** attempt explains that it must be resolved in Source Control and offers to open that view. GitAmida does not add continue, abort, stage, keep, or delete actions.

Keep the built-in Merge Editor as the primary conflict-resolution path. When `merge.tool` or `merge.guitool` is configured, expose **GitAmida: Open in Git Mergetool** only in the title of a standard active live content-conflict editor whose index contains both sides, as a secondary choice before entering the Merge Editor. Hide it while the Merge Editor is active, and do not expose it from Changed-files context menus, modify/delete conflicts, or the Command Palette. Revalidate the repository, path, conflict stages, and unsaved editor state after confirmation, warn that Git mergetool may modify and stage the file or create a backup, invoke only the configured tool for that exact path, and refresh repository state after it exits. This is an explicit external mutation boundary, not a GitAmida conflict resolver.

The virtual-document path follows the current VS Code or Cursor `diffEditor.maxFileSize` setting, whose editor default is 50 MB, instead of imposing a narrower GitAmida-specific text limit. Reclassify changed content when that setting changes, and check the actual endpoint sizes again before reading or decoding them. Image reads likewise obtain the actual Git object size and use that size for the one blob command instead of inheriting the normal 16 MiB Git-output buffer; GitAmida adds no deliberate product limit below the native preview. The editor runtime and available memory remain practical limits because its file-system API returns each image as a complete byte array. Cursor 3.13.25 was unstable when repeatedly loading two approximately 16 MiB custom-scheme BMP endpoints: three of eight full-window loads displayed both sides and five left one side in the native image-loading error state, but the failed sequence eventually recovered after further reloads. The built-in Git scheme also loaded large endpoints slowly. Treat this as native-preview instability for now: do not add an arbitrary lower limit or a temporary-file fallback, and retry by reopening or reloading when it occurs. Pass other binary blobs within the current text-diff size boundary to the native binary fallback without decoding them as UTF-8. Keep submodules and oversized text visible in Changed files and explain why GitAmida does not open those comparisons.

Resolve a canonical standard Git LFS pointer as part of its selected historical endpoint rather than showing the pointer text. Mark it in Changed files from the actual small pointer blob rather than inferring from attributes. Prefer the configured local LFS storage, require a regular object of the declared size, and verify its SHA-256 before passing its bytes to a native diff or external difftool. If an object is missing, fetch only the exact selected path at its parsed commit hash automatically inside a native cancellable progress notification; do not add a confirmation step or size threshold. Disable recent-ref expansion, remote autodetection, and fallback searches; refuse paths whose wildcard syntax cannot be constrained to one file and repositories configured to execute custom LFS transfer commands. Do not run smudge, checkout, pull, repository-wide fetch, or any operation that changes the working tree or index. Extension pointers remain unsupported because honoring them would execute repository-configured transformation commands.

Each native diff is registered with its repository, before and after paths, and both opaque editor URIs before requesting the editor open, then rolled back if that request fails. The editor-title action is scoped directly by the active resource's private `git-amida` or `git-amida-blob` scheme instead of asynchronous global context state, and resolves the exact session from the resource URI supplied by the editor action. It therefore reads the displayed endpoints instead of consulting the current Repository History selection again, so later navigation cannot change the meaning of an already open Range or Selection diff. Cursor's stable Tab API can report no active tab for a native image diff, so each Changed-files row also provides a context-menu fallback. That action validates the path against the Extension Host's currently loaded files and resolves only that file's current single-commit, Range, Selection, or saved working-tree endpoints; it never launches an aggregate or trusts endpoint refs from the Webview. GitAmida writes the two endpoint copies into a private temporary directory and invokes `git difftool --no-index` with an argument array only after the user explicitly requests it. This preserves additions, deletions, renames, working-tree snapshots, image bytes, and unrelated Selection endpoints without asking the external tool to reproduce GitAmida's selection semantics. The configured tool may outlive the launching process, so endpoint copies remain until the Extension Host session ends. Missing or failed tool configuration leaves the native diff open as the fallback.

Virtual text, blob, and native-diff registrations follow their editor tab lifetime. Treat the editor's internal replacement of a text diff with an image editor or binary fallback as the same lifetime while a tab still exposes either endpoint or the complete endpoint pair. Cursor reports that replacement as separate close and open operations, so defer release briefly while the replacement settles; otherwise the binary editor can request an endpoint after GitAmida has removed it. Replacing a preview with another comparison or closing the last matching pinned tab releases both endpoint resources and cancels any blob read still owned by that comparison. A pinned diff retains its resources until its own tab closes, independent of later Repository History or File History navigation.
...
Create local evaluation packages with the exact `@vscode/vsce` development dependency and the repository's `npm run package:vsix` command. The command runs the build first and packages only the JavaScript, Webview assets, manifest, README, changelog, and MIT License declared by the `files` allowlist. Keep the license in every VSIX and retain `vsce` license validation. Local packaging does not require Marketplace credentials or signing, so lifecycle scripts for transitive credential and signing helpers remain explicitly denied. Confirm publisher access and review release metadata separately before public distribution.
...
History queries, file inspection, and diff operations are read-only. Named-branch switching and explicit single-file restoration are GitAmida-owned working-tree mutations; each follows its own preflighted application boundary and neither stages, stashes, discards, forces, or edits Git history. The separately confirmed external mergetool boundary delegates one live content conflict to the user's configured tool and may modify or stage that path as part of Git's standard mergetool flow.
