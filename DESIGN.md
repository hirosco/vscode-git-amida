...
Resolve a canonical standard Git LFS pointer as part of its selected historical endpoint rather than showing the pointer text. Prefer the configured local LFS storage, require a regular object of the declared size, and verify its SHA-256 before passing its bytes to a native diff or external difftool. If an object is missing, require **Download and Open** before network access, then fetch only the exact selected path at its parsed commit hash. Disable recent-ref expansion, remote autodetection, and fallback searches; refuse paths whose wildcard syntax cannot be constrained to one file and repositories configured to execute custom LFS transfer commands. Do not run smudge, checkout, pull, repository-wide fetch, or any operation that changes the working tree or index. Extension pointers remain unsupported because honoring them would execute repository-configured transformation commands.
