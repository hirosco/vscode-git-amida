...
Virtual text, blob, and native-diff registrations follow their editor tab lifetime. Replacing a preview or closing a pinned tab releases both endpoint resources and cancels any blob read still owned by that tab. A pinned diff retains its resources until its own tab closes, independent of later Repository History or File History navigation.
